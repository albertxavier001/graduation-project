...
class GradientNet(nn.Module):
    def build_blocks(self, num_block, num_init_features, pool_ks=3, ks=3):
        bn_size = 4
        growth_rate = 32
        drop_rate = 0
...
#         return features.cuda()
    ...
    def __init__(self, use_gpu=True):
        ...
        i=0; self.denseblock16 = self.build_blocks(self.block_config[i], self.num_input_features[i], ks=7)
        i=1; self.denseblock08 = self.build_blocks(self.block_config[i], self.num_input_features[i], ks=5)
        i=2; self.denseblock04 = self.build_blocks(self.block_config[i], self.num_input_features[i], ks=3)
        i=3; self.denseblock02 = self.build_blocks(self.block_config[i], self.num_input_features[i], ks=3)
        i=4; self.denseblock01 = self.build_blocks(self.block_config[i], self.num_input_features[i], ks=3)
        ...
        """merge v2"""
        self.merge = nn.Sequential()
        self.merge.add_module('merge_denseblock', self.build_blocks((3,3,3), 3*len(self.block_config), pool_ks=1))
