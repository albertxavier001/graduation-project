...
#         return features.cuda()
    ...
    def calOutputChannel(self, input_channel, blocks, bn_size=4, growth_rate=32):
    ...
    def __init__(self, use_gpu=True, bn_size=4, growth_rate=32):
        ...
        for i, blocks in enumerate(self.block_config):
            self.num_upsample_input_features[i] = self.calOutputChannel(self.num_input_features[i], blocks, bn_size=bn_size, growth_rate=growth_rate) 
        ...
        """merge v2"""
        self.merge = nn.Sequential()
        self.merge.add_module('merge_denseblock', self.build_blocks(self.merge_config, 3*len(self.block_config), pool_ks=1, bn_size=bn_size, growth_rate=growth_rate))
        self.merge.add_module('merge_final_conv', nn.Conv2d(in_channels=self.calOutputChannel(3*len(self.block_config), self.merge_config, bn_size=bn_size, growth_rate=growth_rate), out_channels=3, kernel_size=1)) # (3,3,3)->85
