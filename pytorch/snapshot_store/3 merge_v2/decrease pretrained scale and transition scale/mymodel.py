...
#         return features.cuda()
    ...
    def __init__(self, densenet, use_gpu=True, bn_size=4, growth_rate=32, 
                 transition_scale=2, pretrained_scale=1, debug=False):
        ...
        i=0; self.compress16_3ch = nn.Conv2d(self.num_upsample_input_features[i], 3, 1)
        i=1; self.compress08_3ch = nn.Conv2d(self.num_upsample_input_features[i], 3, 1)
        i=2; self.compress04_3ch = nn.Conv2d(self.num_upsample_input_features[i], 3, 1)
        i=3; self.compress02_3ch = nn.Conv2d(self.num_upsample_input_features[i], 3, 1)
        i=4; self.compress01_3ch = nn.Conv2d(self.num_upsample_input_features[i], 3, 1)
        ...
        """merge v2"""
        self.merge = nn.Sequential()
        self.merge.add_module('merge_denseblock', self.build_blocks(self.merge_config, 15, pool_ks=1, bn_size=bn_size, growth_rate=growth_rate, transition_scale=transition_scale))
        self.merge.add_module('merge_final_conv', nn.Conv2d(in_channels=self.calOutputChannel(15, self.merge_config, bn_size=bn_size, growth_rate=growth_rate, transition_scale=transition_scale), out_channels=3, kernel_size=1)) # (3,3,3)->85
    ...
    def forward(self, ft_input):
        ...
        i = 0; ft_predict[i] = self.compress16_3ch(ft_predict[i])
        i = 1; ft_predict[i] = self.compress08_3ch(ft_predict[i])
        i = 2; ft_predict[i] = self.compress04_3ch(ft_predict[i])
        i = 3; ft_predict[i] = self.compress02_3ch(ft_predict[i])
        i = 4; ft_predict[i] = self.compress01_3ch(ft_predict[i])
        if self.debug==True: 
            for i in range(len(ft_predict)): print('after compress', i, ft_predict[i].size())
        ...
        if self.debug==True: 
            for i in range(len(ft_upsampled)): print('after upsample', i, ft_upsampled[i].size())
        ft_concated = torch.cat(ft_upsampled, 1)
        if self.debug == True: print('concat', ft_concated.size())
