...
#         return features.cuda()
    ...
    def __init__(self, use_gpu=True, bn_size=4, growth_rate=32):
        ...
        """merge v2"""
        self.merge = nn.Sequential()
        self.merge.add_module('merge_denseblock', self.build_blocks((3,3,3), 3*len(self.block_config), pool_ks=1, ks=9, bn_size=bn_size, growth_rate=growth_rate))
        self.merge.add_module('merge_final_conv', nn.Conv2d(in_channels=85, out_channels=3, kernel_size=1)) # (3,3,3)->85
