...
#         return features.cuda()
    ...
    def __init__(self, densenet, use_gpu=True, bn_size=4, growth_rate=32, 
        ...
        self.merge_v3_config = [(3,3,3),(3,3,3),(3,3,3),(3,3,3),(3,3,3),(3,3,3)]
        ...
        i=4; self.deconv_01M_to_02M = nn.ConvTranspose2d(self.ch_after_DB[i], self.ch_after_DB[i-1], 4, stride=2, padding=1)
        ...
        self.merge_block_02M = self.build_blocks(self.merge_v3_config[3], self.ch_after_DB[3], ks=3, bn_size=bn_size, growth_rate=growth_rate, transition_scale=transition_scale)
        self.merge_block_04M = self.build_blocks(self.merge_v3_config[2], self.ch_after_DB[2], ks=3, bn_size=bn_size, growth_rate=growth_rate, transition_scale=transition_scale)
        self.merge_block_08M = self.build_blocks(self.merge_v3_config[1], self.ch_after_DB[1], ks=3, bn_size=bn_size, growth_rate=growth_rate, transition_scale=transition_scale)
        self.merge_block_16M = self.build_blocks(self.merge_v3_config[0], self.ch_after_DB[0], ks=3, bn_size=bn_size, growth_rate=growth_rate, transition_scale=transition_scale)
        ...
        i = 3; self.merge_toRGB_02M = nn.Conv2d(self.ch_after_mg[i], final_channel, 1)
        i = 2; self.merge_toRGB_04M = nn.Conv2d(self.ch_after_mg[i], final_channel, 1)
        i = 1; self.merge_toRGB_08M = nn.Conv2d(self.ch_after_mg[i], final_channel, 1)
        i = 0; self.merge_toRGB_16M = nn.Conv2d(self.ch_after_mg[i], final_channel, 1)
         
        i = 5; self.merge_toRGB_32M = nn.Sequential(OrderedDict([
            ('toRGB', nn.Conv2d(64, final_channel, 1)),
            ('sigmoid', nn.Sigmoid()),
        ]))
    ...
    def forward(self, ft_input, go_through_merge=False):
        ...
        # 02M
        i=3;
        ft_predict[i] = self.deconv_01M_to_02M(ft_predict[i+1]) + ft_predict[i]
        ...
        # 04M
        i=2;
        ft_predict[i] = self.deconv_02M_to_04M(ft_predict[i+1]) + ft_predict[i]
        ...
        # 08M
        i=1;
        ft_predict[i] = self.deconv_04M_to_08M(ft_predict[i+1]) + ft_predict[i]
        ...
        # 16M
        i=0;
        ft_predict[i] = self.deconv_08M_to_16M(ft_predict[i+1]) + ft_predict[i]
